 ...
 */
public class BasicNerPipeline_ES
{
	public static void main(String[] args) throws Exception
	{

		Logger logger = Logger.getLogger(BasicNerPipeline_ES.class.toString());
		...
		CollectionReaderDescription reader = createReaderDescription(BaseCasReader.class,
				...
				BaseCasReader.PARAM_QUERY,"{'meta.source.inLanguage':'es','processing.available_data': {'$ne': 'text_nerl'}}",
				// BaseCasReader.PARAM_QUERY,"{'meta.source.inLanguage':'es'}", // reprocess everything
				BaseCasReader.PARAM_LANG,"{'$literal':'es'}"
				);

		AnalysisEngineDescription segmenter = createEngineDescription(LanguageToolSegmenter.class);

		AnalysisEngineDescription dbpedia = createEngineDescription(SpotlightAnnotator.class,
				SpotlightAnnotator.PARAM_ENDPOINT, "http://localhost:2224/rest",
		...
		AnalysisEngineDescription key = createEngineDescription(KeyPhraseAnnotator.class,
				KeyPhraseAnnotator.PARAM_LANGUAGE, "es",
